...
| 依赖 | 说明 | 安装方式 |
|---|---|---|
| Python | **推荐 3.12 或 3.13**——这两个版本已实测部署通过，且都带哈希锁文件。脚本不限制版本：装了别的版本照样会往下走，只是没有锁文件时改用 `requirements.txt` 安装，不锁版本也没有哈希校验，脚本会明确提示。3.14 目前装不上，因为本项目钉的 `numpy==2.2.6` 还没有 3.14 的预编译包 | Windows `winget install Python.Python.3.12`；Linux 用发行版包管理器 |
| ffmpeg + ffprobe | 事件录像的 H.264 证据链必需，两个都要 | Windows `winget install Gyan.FFmpeg`；Debian/Ubuntu `sudo apt-get install -y ffmpeg` |
...
| 提示未检测到 ffmpeg/ffprobe | 没装或不在 PATH | 按 4.1 安装后**重开终端** |
| 装依赖中途失败 | 多数是网络问题 | 加 `-PipMirror` / `--pip-mirror` 重试 |

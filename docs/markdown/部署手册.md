...
Windows 的 `start.ps1` 全程写 `start-log.txt`，把这一个文件发给技术支持即可。Linux 侧用：
...
| 启动报 `OSError: [WinError 1114]`，指向 `torch\lib\c10.dll` | 系统缺 MSVC 运行库（`c10.dll` 依赖 `MSVCP140` / `VCRUNTIME140` / `VCRUNTIME140_1`）。**与显卡无关**，CPU 版同样会遇到 | `winget install --id Microsoft.VCRedist.2015+.x64` 或装 <https://aka.ms/vs/17/release/vc_redist.x64.exe>，装完重跑，依赖不用重装 |

...
| 2 | 校验交付文件 | `verify.ps1` / `verify.sh` | CPU 约 1 分钟，GPU 约 3 分钟 |
| 3 | 运行启动脚本 | `start.ps1` / `start.sh` | CPU 约 5 分钟，GPU 约 15 分钟 |
| 4 | 打开 Web 页面确认就绪 | 浏览器访问 `http://127.0.0.1:8600` | 1 分钟 |
| 5 | 选择视频接入模式 | `configure-go2rtc.*`，见第 4 章 | 5 分钟起 |

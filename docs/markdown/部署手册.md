...
只有一处：**内置 go2rtc 模式**在源码部署下需要你自己提供 go2rtc 可执行文件，并让它运行在
`.env` 里 `SMOKEFIRE_GO2RTC_API` 指向的地址上——Docker 版是随包带了 go2rtc 容器的。
另外两种视频接入模式（直接 RTSP、对接现场已有的 go2rtc）不受影响，用法与第 6、7 章完全一致。

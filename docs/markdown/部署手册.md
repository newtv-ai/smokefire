...
源码包体积小得多，但目标机需要自备 Python 3.12 与 ffmpeg，且**首次安装依赖需要联网**（CPU 约 1 GB，GPU 约 3 GB）。详见第 4 章。
...
| | Docker 部署 | 源码部署 |
|---|---|---|
| 目标机要装 | Docker + Compose v2 | Python 3.12、ffmpeg、（Linux 还要 libgl1） |
...
| 依赖 | 说明 | 安装方式 |
|---|---|---|
| Python 3.12 | 锁文件按 3.12 生成，其它版本会被拒绝 | Windows `winget install Python.Python.3.12`；Linux 用发行版包管理器 |
...
D:\smokefire-source  smokefire\              程序源码
  ...
  deploy\                 安装与服务注册脚本
  docs\                   中英文手册 PDF
  requirements-*.lock      四个平台锁文件，脚本自动选
  .env.example             配置模板
...
| 现象 | 原因 | 处理 |
|---|---|---|
| 提示锁文件要求 Python 3.12 | 装的不是 3.12 | 装 3.12 并确认 `python --version` |
